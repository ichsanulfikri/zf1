	...
	public function getPaginateData($search=null){
		...
		if($search){
			$selectData = $db->select()
						...
						if( isset($search['sm_name_bahasa'])){
							$selectData->where("sm.sm_name_bahasa LIKE '%".$search['sm_name_bahasa']."%'");
						}
						
						if( isset($search['sm_address1'])){
							$selectData->where("sm.sm_address1 LIKE '%".$search['sm_address1']."%'");
						}
						
						if( isset($search['sm_address2'])){
							$selectData->where("sm.sm_address2 LIKE '%".$search['sm_address2']."%'");
						}
						
						if( isset($search['sm_email'])){
							$selectData->where("sm.sm_email LIKE '%".$search['sm_email']."%'");
						}
						
						if( isset($search['sm_url'])){
							$selectData->where("sm.sm_url LIKE '%".$search['sm_url']."%'");
						}
						
						if( isset($search['sm_phone_o'])){
							$selectData->where("sm.sm_phone_o LIKE '%".$search['sm_phone_o']."%'");
						}
