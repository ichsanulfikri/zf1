 ...
 public function fngetProgramData($program_id=0) { //Function to get the user details
  	...
  	if($program_id!=0){
  		$lstrSelect = $this->lobjDbAdpt->select()
                    ...
                    ->joinLeft(array('col' => 'tbl_collegemaster'), 'col.IdCollege=a.IdCollege',array('collegeName'=>'col.ArabicName','collegeShortcode'=>'col.ShortName'))
                    ->joinLeft(array('univ'=>'tbl_universitymaster'), 'univ.idUniversity = col.affiliatedto', array('univ_mohe_code'=>'Univ_code_EPSBED'))
                    ->order("a.ProgramCode");
  ...
  public function fngetFacultyProgramData($faculty_id=0) { //Function to get the user details
  	 
  	if($faculty_id!=0){
  		$lstrSelect = $this->lobjDbAdpt->select()
  		->from(array("a" => "tbl_program"))
  		->where("a.Active = 1")
  		->where("a.IdCollege =?", $faculty_id)
  		->joinLeft(array('col' => 'tbl_collegemaster'), 'col.IdCollege=a.IdCollege',array('collegeName'=>'col.ArabicName','collegeShortcode'=>'col.ShortName'))
  		->joinLeft(array('univ'=>'tbl_universitymaster'), 'univ.idUniversity = col.affiliatedto', array('univ_mohe_code'=>'Univ_code_EPSBED'))
  		->order("a.ProgramCode");
...
public function getProgramDetails() {
 ...
 public function getAllProgram() {
    ...
    $lstrSelect = $lobjDbAdpt->select()
                    ...
                    ->order("pm.ProgramCode");
    $larrResult = $lobjDbAdpt->fetchAll($lstrSelect);
    return $larrResult;
  }
	
  
	public function getProgramByfacultyandscheme($IdFaculty,$IdScheme){
		$lstrSelect = $this->lobjDbAdpt->select()
		->from(array("a" => "tbl_program"), array("key" => "a.IdProgram", "name" => "ProgramName"))
		->joinLeft(array("b" => "tbl_program_scheme"), "a.IdProgram = b.IdProgram",array(""));
		if(($IdFaculty == 0) && ($IdScheme!= 0)){
			$lstrSelect->where("b.IdScheme = ?", $IdScheme);
		}
	...
	public function getProgramDataByfacultyandscheme($IdFaculty,$IdScheme){
		$lstrSelect = $this->lobjDbAdpt->select()
		->from(array("a" => "tbl_program"))
		->joinLeft(array("b" => "tbl_program_scheme"), "a.IdProgram = b.IdProgram",array(""))
		->order('a.ProgramCode');
	...
	public function fnDeleteProgramScheme($formData,$program_id) {//function to delete from  database
		$db = Zend_Db_Table::getDefaultAdapter();
		if ($formData['delete_scheme']) {
			$id = $formData['delete_scheme'];
			$arrid = explode(",", $id);
			for ($i = 0; $i < count($arrid); $i++) {
				$where = "IdProgramScheme = ".$arrid[$i];
				$db->delete('tbl_program_scheme', $where);
			}
			return;
		}
		return;
	...
	public function fninsertProgramScheme($formData, $program_id) {  // function to insert po details
		$db = Zend_Db_Table::getDefaultAdapter();
		$table = "tbl_program_scheme";
		if (isset($formData['IdScheme_add'])) {
			$scheme_list = $formData['IdScheme_add'];
			for ($i = 0; $i < sizeof($scheme_list); $i++) {
				$data = array(
						'IdProgram' => $program_id,
						'IdScheme' => $formData['IdScheme_add'][$i],
				);
