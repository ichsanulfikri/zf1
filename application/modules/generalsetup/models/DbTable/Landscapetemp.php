...
class GeneralSetup_Model_DbTable_Landscapetemp extends Zend_Db_Table_Abstract
{
	...
	public function fnDeleteTemplandscapeYearBysession($sessionID){
		$db = Zend_Db_Table::getDefaultAdapter();
	...
	public function fninserttempblocksubject($templandscapeblocksublist,$sessionID) {  // function to insert po details
		...
		foreach($templandscapeblocksublist as $formData) {
			$larrtepprogrmentryreq = array('IdLandscape'=>$formData['IdLandscape'],
										   ...
										   'coursetypeid'=>$formData['IdCourseType'],
										   'session_id'=>$sessionID,
										   'deleteFlag'=>'1');
	...
	public function fninserttempblockyearsem($templandscapeblockyearlistsem,$sessionID){
			$lobjdb = Zend_Db_Table::getDefaultAdapter();
			$table = "tbl_templandscapeblockyear";
			$sessionID = Zend_Session::getId();

			foreach($templandscapeblockyearlistsem as $formData) {
				$larrtepprogrmentryreq = array('IdLandscape'=>$formData['IdLandscape'],
											   'Year'=>$formData['Year'],
				...
				$lobjdb->insert($table,$larrtepprogrmentryreq);
			}
	}
    ...
    public function fnGetTempLandScapeBlockYearDtls($idlandscape,$sessionID){
    	$db = Zend_Db_Table::getDefaultAdapter();
		$select = $db->select()
				->from(array('lby'=>'tbl_templandscapeblockyear'),array('lby.*'))
   ...
   public function fnLandscapeList($IdProgram) { //Function for the view University
	...
	public function fninsertLandscape($formData) {  // function to insert po details
		if(!isset($formData['Blockcount'])){
			$formData['Blockcount'] = 0;
		}
		$db = Zend_Db_Table::getDefaultAdapter();
			$table = "tbl_landscape";
			$larrLandscapetempDetails = array('IdProgram'=>$formData['IdProgram'],
											  ...
											  'landscapeDefaultLanguage'=>$formData['landscapeDefaultLanguage'],
											  'AddDrop'=>$formData['AddDrop'],
...
/*	public function fninsertLandscapeBlockSubject($formData,$resultLandscape){  // function to insert po details
	...
	public function fninsertLandscapeBlockSubject($formData,$resultLandscape){

			$db = Zend_Db_Table::getDefaultAdapter();
			$table = "tbl_landscapeblocksubject";
			$gridcount = count($formData['blocksubnamegrid']);
			for($i=0;$i<$gridcount;$i++){
				$data = array('idlandscape' => $resultLandscape,
							  'blockid' => $formData['blocksubnamegrid'][$i],
							  'subjectid' => $formData['subjectblocknamegrid'][$i],
							  'coursetypeid' => $formData['coursetypegrid'][$i]);
	...
	public function fninsertLandscapeBlockSemester($formData,$resultLandscape){  // function to insert po details
			...
			$gridcount = count($formData['SemNameListnamegrid']);
			for($i=0;$i<$gridcount;$i++){
				$data = array('idlandscape' => $resultLandscape,
		...
		/**
		 *
		 * function to insert blocklevel landscape year details
		 * @param $formData
		 * @param $resultLandscape
		 */
   public function fninsertLandscapeBlockYear($formData,$resultLandscape){
   	$db = Zend_Db_Table::getDefaultAdapter();
			$table = "tbl_blocksemesteryear";
			$gridcount = count($formData['blockyeargrid']);
			for($i=0;$i<$gridcount;$i++){
				$data = array('IdLandscape' => $resultLandscape,
							  'Year' => $formData['blockyeargrid'][$i],
							  'YearSemester' => $formData['yearblocksemgrid'][$i]);
