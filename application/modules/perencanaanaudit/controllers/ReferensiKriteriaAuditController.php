<?php 
class Perencanaanaudit_ReferensiKriteriaAuditController extends  Base_Base
{
...
//     $this->view->hello="hello world";
    ...
    {
        $db = new Perencanaanaudit_Model_DbTable_Kriteria();
        if ($this->_request->isPost())
        {
            ...
            if (isset($fromData['Save']))
            {
                if ($id)
                {
                    ...
                    $this->_redirect('/perencanaanaudit/referensi-kriteria-audit');
                }
            }
        } else 
        {
